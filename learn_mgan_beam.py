...
if __name__=='__main__':
    ...
    parser.add_argument("--m_lambda", type=float, default=0.05, help="monotone penalty constant")
    ...
    ny = umbridge_model.get_output_sizes()[0]  # outout is the deflection for each finite element (using 30 instead of 31)
    ...
    x_train = my_prior.rvs(N_train)  # num_rows = N_train ; num_cols = nx = 31 --> x_train is log_E_true
    
    y_train = np.zeros(shape=(N_train, ny-1))  # num_rows = N_train ; num_cols = ny = 30
    for i in range(N_train):
        prior_sample = x_train[i]  #log_E sample
        y_at_sample = my_model(prior_sample)  #d_sample

        sigma_noise = 1e-3
        noise = np.random.normal(loc=0, scale=sigma_noise, size=ny) 

        y_train[i] = y_at_sample[1:] + noise[1:]  #adding noise to generate observations

    x_train = torch.tensor(x_train).float()
    y_train = torch.tensor(y_train).float()
    
    # save data
    scipy.io.savemat(f'{dataset}_training_data_{N_train}.mat',mdict={'x_train':x_train.detach().numpy(), 'y_train':y_train.detach().numpy()})
    ...
    for epoch in range(args.n_epochs):
        ...
        for (x,y) in tqdm(xy_loader):
            ...
            monp = monotonicity_penalty(fake_2, fake_prime, fake_noise_2, fake_noise_prime)
    ...
    plt.legend()
    plt.savefig(f'losses_{dataset}_{N_train}.pdf')
    plt.close()
    ...
    plt.ylim(-1.5,1.5)
    plt.savefig(f'gradient_penalty_{dataset}_{N_train}.pdf')
    plt.close()
    ...
    plt.savefig(f'monotonicity_penalty_{dataset}_{N_train}.pdf')
    plt.close()
    
    # save map
    name = f'mgan_{dataset}_{N_train}_gen.pt'
